...
final class VerveAdapter: PartnerAdapter {
    ...
    var configuration: PartnerAdapterConfiguration.Type { VerveAdapterConfiguration.self }
    ...
    func setConsents(_ consents: [ConsentKey: ConsentValue], modifiedKeys: Set<ConsentKey>) {
        ...
        if modifiedKeys.contains(ConsentKeys.tcf) {
            if let tcfString = consents[ConsentKeys.tcf] {
                ...
                HyBidUserDataManager.sharedInstance().removeIABGDPRConsentString()
                log(.privacyUpdated(setting: "IABGDPRConsentString", value: nil))
            }
        }
        ...
        if modifiedKeys.contains(ConsentKeys.usp) {
            if let uspString = consents[ConsentKeys.usp] {
                ...
                log(.privacyUpdated(setting: "IABUSPrivacyString", value: nil))
            }
        }

        // GPP
        if modifiedKeys.contains(ConsentKeys.gpp) {
            if let gppString = consents[ConsentKeys.gpp] {
                HyBidUserDataManager.sharedInstance().setPublicGPPString(gppString)
                log(.privacyUpdated(setting: "PublicGPPString", value: gppString))
            } else {
                HyBidUserDataManager.sharedInstance().removePublicGPPString()
                log(.privacyUpdated(setting: "PublicGPPString", value: nil))
            }
        }
    }
